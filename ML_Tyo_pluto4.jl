...
# ╔═╡ 0d2af912-e678-4691-9e42-f5dcc1a00204
begin 
	using Plots
	gr()
end

# ╔═╡ d57c781f-bc6b-463d-b59c-ea550e74db44
using Polynomials

# ╔═╡ 9a4b7d23-dcf0-41f8-a48f-4ab8a3e0d9d7
using LaTeXStrings

# ╔═╡ 9f6cc62b-93c6-4e46-86a2-83861366e0a0
using Latexify

# ╔═╡ 938bf2ce-7426-11ec-1958-d19aaf822c02
md"""
# 2.重回帰
## 2.1. 重回帰
"""

# ╔═╡ 79daaae4-125e-422d-aefa-aca3e0493fea
begin
	a, b = 0.431, 3.310
	D = [
		1 3
...
# ╔═╡ 257c9da2-2ddb-4842-a90b-46b72f2b81eb
begin
	struct MRShow end 
	@recipe function f(::MRShow, X::AbstractVector, Y::AbstractVector)
...
# ╔═╡ 968fcd0e-85d0-4941-a805-61b1edfd4a16
function plot_ŷ(a, b)
	...
	label = (a == 1) ? "x + $(b))" : "$(a)x + $(b)" 
	plot!(x -> f(x, a, b); c=:red, lw=1, label=label)
	
end

# ╔═╡ 9f4869ba-4f4f-4edd-bba2-c424c6df8860
function plot_error(a, b, X, Y)

	f(x, a, b) = a*x + b
	Ŷ = f.(X, a, b)
		
	for i in 1:length(X)
		plot!([(X[i], Ŷ[i]), (X[i], Y[i])]; c=:blue, ls=:dash, label="")
	end
	plot!()
end

# ╔═╡ 903395e9-ce9b-43fc-8eab-7d6f880e8e1a
let
	gr()
	plot(MRShow(), X, Y;)
	plot_error(a, b, X, Y;)
...
# ╔═╡ c4db6342-8e42-4b7d-b7e7-9927ee5de79a
function plot_ŷ_poly(W)

	f = Polynomial(reverse(W))

	label = latexstring(latexify("$f", env=:raw, cdot=false) )
	plot!(x -> f.(x); c=:red, lw=1, label=label, legendfontsize=9)
	
end

# ╔═╡ e103eabc-8908-4950-b533-e93c5bf91b75
let
	gr()
	plot( MRShow(), X, Y;)

	W = [0.09, -1.27, 5.41, -1.23]
	plot_ŷ_poly(W;)
end

# ╔═╡ c45e5902-7823-44b4-896f-5bc5261fc906
md""" ## 2.9. 重回帰の実施例 """

# ╔═╡ e52e6a48-9bb6-4554-a42e-74e9d915e1f3
begin
	Xs = [
...
# ╔═╡ 99c118ff-1af8-4ffe-b1ad-dbd8b835e152
begin
	struct MRShow2 end 
	@recipe function f(::MRShow2, X::AbstractVector, Y::AbstractVector)

		framestyle := :box
		# aspect_ratio := :equal
		xlabel     :=  "最高気温の月平均(ᵒC)"
		ylabel     :=  "支出額(円)"
		title      :=  "最高気温とアイスクリーム・シャーベットの支出額"
		titlefontsize := 12
		size       --> (600, 400)
		xlims      --> (0, 35.5)
		ylims      --> (-255, 2000)
...
# ╔═╡ ae8cff80-279e-45e4-b479-98b47f8e038c
function plot_polyfit(X, Y, deg)

	f = fit(X, Y, deg)

	label ="$(deg)次関数"
	# label = latexstring(latexify("$f", env=:raw, cdot=false) )
	plot!(x -> f.(x); c=:red, lw=2, label=label, legend=:topleft)
	
end

# ╔═╡ acd9578f-985a-4949-b19b-b3ba0091c915
let
	plotly()
	plot(MRShow2(), Xs, Ys;)
	plot_polyfit(Xs, Ys, 2)
end

# ╔═╡ e7204b09-33f8-4d52-b9b5-01a009afa26f
let
	plotly()
	plot(MRShow2(), Xs, Ys;)
	plot_polyfit(Xs, Ys, 3)
end

# ╔═╡ d409b213-918c-4281-8c44-b013d478dc15
let
	plotly()
	plot(MRShow2(), Xs, Ys;)
	plot_polyfit(Xs, Ys, 4)
end

# ╔═╡ 2979c4da-51b8-441e-ae87-c3d54d416667
#
# using GLM
#

# ╔═╡ 06ddcbb6-128d-495f-8bc8-79f10b39e99f
