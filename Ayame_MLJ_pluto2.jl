...
# ╔═╡ 3bf977e6-c6bc-11ec-163b-dff6d04fe7f2
begin
	using MLJ
	using CSV
	using DataFrames
...
# ╔═╡ b54bdfda-eb13-4203-a2fa-17ef7e10e6d4
using MLJLinearModels

# ╔═╡ 7a300d6b-7d80-42b1-bd0d-143ebb2fe6f7
md"# SIGNATEのアヤメ問題を解く"

# ╔═╡ 1087b496-507c-4c90-aebe-a88f1151761f
md"### ライブラリの読み込み"

# ╔═╡ 90012cc8-ba90-4690-aa08-129fbf0ff3aa
md"### データの読み込みと内容確認"

# ╔═╡ 231f9a81-d888-4e75-a20e-f8d409654908
begin
	train = CSV.read("./train.tsv", DataFrame, delim="\t")
	test  = CSV.read("./test.tsv", DataFrame, delim="\t")
	sub   = CSV.read("./sample_submit.csv", DataFrame, header=false)
end;

# ╔═╡ 420a18e6-4e82-4447-a982-bd25717fe4fb
first(train, 3) |> pretty

# ╔═╡ b8053eba-34d2-45d5-bf48-223a5dd53f03
first(test, 3) |> pretty

# ╔═╡ ab2b6837-70e9-4aad-b309-84d4d25d714e
first(sub, 3) |> pretty

# ╔═╡ f5ff3a0c-1201-4050-a14a-d5f57cb3425c
size(train), size(test), size(sub)

# ╔═╡ 8cc410c8-41c7-42de-ac47-6c0e1c95f665
LocalResource("./iris.JPG")

# ╔═╡ af44f674-3cfa-4ea1-8410-4f40f3ab8551
describe(train)

# ╔═╡ 3da3e261-0871-4eda-a97f-50884844d41d
describe(test)

# ╔═╡ 00ddf5cc-16ba-4bb3-8116-92f54d8fb9ff
describe(sub)

# ╔═╡ 9d436d07-26ec-4bcd-9fcd-7a9da80173e2
combine(groupby(train, [:class]), nrow => :count)

# ╔═╡ 235edcbe-baca-44a9-9ba9-3e61772e784f
begin
	gdf = groupby(train, :class)
	combine(gdf, names(train, Not([:id, :class])) .=> std)
end

# ╔═╡ 1291b6ec-baae-43c4-a9b0-3e4f9bbf37b6
length(gdf)

# ╔═╡ 5a7e9d6b-19eb-48f8-8850-3f4c7e9fab92
function pairplot(df)
	dim = size(df, 2) - 2
	f = Figure(resolution=(900, 800))

	axes = [Axis(f[i, j]) for j in 1:dim, i in 1:dim]
	num = 0
	...
	for i in 1:dim, j in 1:dim
		...
		if i == j
			num += 1
			# hidedecorations!(axes[num], grid = false)
			for k in 1:length(gdf)
				x = groupby(train, :class)[k][:,j+1]
				y = groupby(train, :class)[k][:,i+1]
				density!(axes[num], x)
			end
		else
			num += 1
...
# ╔═╡ b333f59f-ec98-4cfe-8229-a5c1c4f07ea1
pairplot(train)

# ╔═╡ cf3268d4-ebcc-4752-abd1-9a786164145a
md"### class をエンコードする"

# ╔═╡ b5ba1c29-15f0-4a93-8fa9-f58eb493c44c
# begin
# 	ua = unique(train[:, :class])
# 	y_train = [findfirst(==(x), ua) for x in train[:, :class]]
# end

# ╔═╡ 1bbcce66-fcb3-4669-a8e9-988996dbabce
y, X = unpack(train, ==(:class))

# ╔═╡ 2c4bfd50-aeb8-4fa2-b9d4-b58426ade650
X

# ╔═╡ 4dd99be1-58b3-4fdc-93c9-d179c403bd03
md"### ロジスティック回帰"

# ╔═╡ f1371cb8-b09f-474a-b0b8-ae95ee82d1dc
begin
	LogisticClassifier = @load LogisticClassifier pkg=MLJLinearModels
	lc_model = LogisticClassifier()
end

# ╔═╡ 94087b95-a8c3-464a-a2a4-b5bbac110e3c
lc = machine(lc_model, X[:, 2:end], categorical(y))

# ╔═╡ d1157384-37e5-4e3d-b9d7-ec939d1ae434
begin
	rng = StableRNG(566)
	my_train, my_test = partition(eachindex(y), 0.6, shuffle=true, rng=rng)
end

# ╔═╡ b6b198ab-ccf7-4b73-9aea-2fa1c875c768
my_test[1:3]

# ╔═╡ ab78e8c7-90c5-4fda-8c52-bf320d1b9556
fit!(lc, rows=my_train)

# ╔═╡ 4fc7cf5c-21fd-46bc-b58a-13c285e8f0e6
fitted_params(lc) |> pprint

# ╔═╡ 8b24363a-b1bc-4bc8-a27f-422cbff00055
ŷ = predict(lc, rows=my_test)

# ╔═╡ 38071c50-7927-4c83-8394-9a51211b79bf
@show ŷ[1]

# ╔═╡ acc96cdb-e6a5-4ca0-9075-7d517097575b
ŷ

# ╔═╡ 8d61e08f-18df-498d-8086-060a6bdd36cc
begin
	mce = cross_entropy(ŷ, y[my_test]) |> mean
	round(mce, digits=4)
end

# ╔═╡ 38da2a83-953c-4266-a87e-829e2bc32693
typeof(ŷ)
...
git-tree-sha1 = "7f3aeaf1e6e37fe0f309eacc1ccc6547ac21e719"
...
git-tree-sha1 = "d7c73ba92288af6c34504738f5345b10f174474b"
