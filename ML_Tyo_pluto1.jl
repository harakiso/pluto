...
# ╔═╡ 846606e8-6460-11ec-1a5d-d3d23ef4f673
begin
	using PlutoUI
	using Plots
	gr()
...
# ╔═╡ 846606d4-6460-11ec-102b-5d8a8d0692c3
html"""
...
# ╔═╡ ec77b247-d413-4be4-af1b-ebb2c77ddf4f
md""" # 機械学習帳
...
# ╔═╡ 112e2cbf-df3a-4a5e-bee6-415416d11292
begin
	mmt = [
...
# ╔═╡ 28c49815-af69-4a09-bee0-7e0e5b794405
let
	plot(title="MMT vs. Spend for Icecream", framestyle=:box)
	plot!(xlabel="month mean maximum temperature [̇Degree]", ylabel="spend [Yen]")
	plot!(xlim=(0,35), ylim=(0,2000), yticks=0:250:2000, gridalpha=0.5)
	scatter!(mmt, spend; marker=:dot, ms=3, label="")
end

# ╔═╡ 97486724-dadc-4f37-989e-2f4a5d0df311
let
	plot(title="MMT vs. Spend for Icecream", framestyle=:box, legend=false)
	plot!(xlabel="month mean maximum temperature [̇Degree]", ylabel="spend [Yen]")
	plot!(xlim=(0,35), ylim=(0,2000), yticks=0:250:2000, gridalpha=0.5)
	scatter!(mmt, spend; marker=:dot, ms=3, label="")
	...
	samples = [0, 35]
	sl(X, a, b) = a * X + b
	plot!(samples, sl.(samples, 15, 500); ls=:dash, lc=:red, lw=2)
	plot!(samples, sl.(samples, 25, 250); ls=:dot, lc=:red, lw=2)
	plot!(samples, sl.(samples, 40, 0); ls=:dashdot, lc=:red, lw=2)
	annotate!([(20,1000,text("?", 18, :center, :top, :red))])
end

# ╔═╡ 582119fd-532c-4449-a725-b92bdfb7ae87
md"### 1.2.データの表現"

# ╔═╡ 8fb12807-f107-44e1-a349-cb1c9cd4e33c
begin
	D = [
		1 3
		3 6
...
# ╔═╡ 092927fe-abb9-4ba2-a2a0-aa6698078dfc
p1 = let
	...
	texts = ["(x₁,y₁)", "(x₂,y₂)", "(x₃,y₃)", "(x₄,y₄)"]
	for i in 1:length(colormap)
		annotate!([(X[i],Y[i]+offset_y[i],text(texts[i], 10,
			:center, colormap[i]))])
	end
	plot!()
end

# ╔═╡ 3107108c-9f17-404c-809b-a5a87e162965
md"### 1.3.残差
残差(residual) $ϵ_{i}$は、
```math
	\epsilon_i = y_i - ŷ_i = y_i - (ax_i + b)
```
となる。
"

# ╔═╡ 21406b95-ac22-44f7-b8df-2db642eccf56
function plot_ŷ()
# p2 = let
	plot(p1)

	colormap = [:red, :blue, :green, :orange]
	offset_ŷ = [-0.8, -0.8, 0.6, 0.6]
...
# ╔═╡ f0846643-10b6-4f66-99d8-e86d69c9886f
function plot_error()
	colormap = [:red, :blue, :green, :orange]
...
# ╔═╡ 25802987-7a46-4f93-b494-5f9815f8fd52
let
	plot_ŷ()
	plot_error()
end

# ╔═╡ fc8fea2d-d8dd-49f6-99ad-d48d8481505a
function plot_msr(best, aa, bb)
	...
	msr = ( Y - (aa .*X .+ bb) ).^2 |> mean
#	if isequal(best[1], NaN) || msr < best[1]
#		best = [msr, aa, bb]
#		plot!(x -> f(x, best[2], best[3]); c=:red, lw=1.5, label="")
#	end


	scatter!(D[:,1], Ŷ; c=colormap, shape=:star5, ms=6, markerstrokewidth=0, label="")
	plot!(x -> f(x, aa, bb); c=:black, lw=1.5, label="")

	for i in 1:length(colormap)
		plot!([(X[i], Ŷ[i]), (X[i], Y[i])]; c=colormap[i], ls=:dash, label="")
	end
	
	msr_r = round(msr; digits=2)
	msg = "a = $(aa), b = $(bb): MSR = $(msr_r)"
	msr_b = round(best[1]; digits=2)
	msg2 = "(Current best: a = $(best[2]), b = $(best[3]): MSR = $(msr_b))"
    annotate!([(5.0,1.0,text(msg*"\n"*msg2, 10, :center, :black))])
end

# ╔═╡ 94f6f48a-6127-4433-9d8c-fbbb0e6fdbec
begin
	a_slider = @bind aa Slider(range(0, 1, length=21), show_value=true)
	b_slider = @bind bb Slider(range(0, 6, length=13), show_value=true)
...
# ╔═╡ 5794e4d6-6260-407e-93eb-82172bf97a12
let
	best = [NaN, NaN, NaN]
	plot_msr(best, aa, bb)
end

# ╔═╡ 94e37763-a182-4a13-913b-c6c35be09db8
let
	...
	a_range = range(0, 1, length=21)
	b_range = range(0, 6, length=13)
	
	anim = @animate for bb in b_range, aa in a_range
		...
		# 回帰直線のプロット点と直線
		scatter!(D[:,1], Ŷ; c=colormap, shape=:star5, ms=6, 
		...
		if isequal(best[1], NaN) || msr < best[1]
			...
			plot!(x -> f(x, best[2], best[3]); alpha=0.5, c=:red, lw=1.5, label="")
		end
		...
		msr_b = round(best[1]; digits=2)
		msg2 = "(Current best: a = $(best[2]), b = $(best[3]): MSR = $(msr_b))"
	    annotate!([(5.0,9.0,text(msg*"\n"*msg2, 10, :center, :black))])
			
	end
	gif(anim, "msr.gif"; fps=2)
end

# ╔═╡ 932ef61c-e812-445e-9875-2efcc58de3ea
J = calc_msr(a, b', D)
