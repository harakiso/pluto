...
let
	...
	y1 = 5/6 * x
	y2 = 3x
	plot(xlim=(0,10), ylim=(0,10), xticks=0:2:10, yticks=0:2:10,
		framestyle=:box, aspect_ratio=true, legend=false)
	plot!(xlabel="x", ylabel="y")
	plot!(x, y1; c=:black, fill=(0, 0.3, :green))
	plot!(x, y2; c=:black, fill=(10, 0.3, :green))
	scatter!(D[:,1], D[:,2]; markerstrokewidth=0, ms=5, c=:blue)

	annotate!([(6, 6,text("a=5/6", 10, :center, :black))])
	annotate!([(3.3, 8,text("a=3", 10, :center, :black))])
...
let
	...
	plot(xlim=(-11.0, 11.0), ylim=(-12.0, 11.0), framestyle=:box)
	plot!(xlabel="x", ylabel="y", legend=false)
	scatter!(X, Y; markerstrokewidth=0, ms=4, c=:blue)
	scatter!(nX, nY; markerstrokewidth=0, ms=6, c=:blue, shape=:star6)
