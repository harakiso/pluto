...
# ╔═╡ 9a4296b0-a45d-4dc0-beac-fac228e5b4f1
begin
	using CSV
	using DataFrames
	using ScikitLearn
...
# ╔═╡ a4a8c805-1b86-45a9-837b-1bc3a85bfd61
using Dates

# ╔═╡ 976f1611-2adb-4495-ac38-1c386a562f99
using PlotUtils: optimize_ticks

# ╔═╡ 602589a6-f991-4fc1-8316-8288cb32cee1
using HypothesisTests

# ╔═╡ 9a6ef11e-da3c-11ec-002f-819b44d85d0d
md"# 【練習問題】お弁当の需要予測
[ここを参考にした、【練習問題】お弁当の需要予測](https://signate.jp/competitions/24/tutorials/3)
...
# ╔═╡ 0801026a-085c-4ad1-85bc-95a8dd2ce411
md"## はじめに
...
# ╔═╡ b696755d-d8c1-4f7f-99cc-1ce54c1bc8fe
md"### ライブラリのロード"

# ╔═╡ df32b81a-82af-4a28-b938-3b81c91d306a
md"""
| カラム | ヘッダ名称  | データ型 | 説明                               |
|-------|-----------|---------|----------------------------------- | 
| 0	    | datetid	| datetime| インデックスとして使用する日付(yyy-m-d) |
| 1	    | y         | int     | 販売数（目的変数）                     |
| 2	    | week      | char    | 曜日（月～金）                         |
...
# ╔═╡ 4467a948-aec8-48b6-8e89-028ebdf5d86b
md"### データの読み込み"

# ╔═╡ 4964046a-193a-4cba-a615-6477d57db893
begin
	train = CSV.read("./train.csv", DataFrame)
	test = CSV.read("./test.csv", DataFrame)
	sample = CSV.read("./sample.csv", DataFrame)
	println("Data Shapes")
	println("Train:", size(train), " Test:", size(test), " Sample:", size(sample))
end

# ╔═╡ c44943b0-eaa4-440a-972f-64a183cdb646
first(train, 5)

# ╔═╡ 79c78e6e-9934-491f-bf62-b1aad14be77c
describe(train)

# ╔═╡ fd11d056-6934-400c-9326-74f56cc6ef4c
md"欠損値の補間と月情報の特徴量の作成"

# ╔═╡ 04342097-d85b-4b68-a6c5-48133b0019ef
begin
	# 給料日ではない日を 0 で置き換え
	...
	replace!(train.remarks, missing => "なし")
	# 月情報の特徴量を、datetimeから切り出して追加
...
let
	dateticks = optimize_ticks(train.datetime[1], train.datetime[end])[1]
	fig = Figure(backgroundcolor=:gray95, resolution=(1200, 500))
	ax1 = Axis(fig[1,1])
...
let
	...
	for i in 1:length(features)
		axes[i].ylabel = "販売数（目的変数）"
		axes[i].xlabel = string(features[i])
		scatter!(axes[i], train[:, features[i]], train.y, color=:blue)
	end
...
# ╔═╡ 51351429-af6e-4670-97c4-68297345bcb0
function categorical_2_num(elements)
	nums = Int[]
	un = unique(elements)
...
let
	...
	train.week_num = categorical_2_num(train.week)
	train.weather_num = categorical_2_num(train.weather)
	train.remarks_num = categorical_2_num(train.remarks)
	train.event_num = categorical_2_num(train.event)
	
	fig = Figure(backgroundcolor=:gray95, resolution=(1000, 800))
	grid = fig[1,1] = GridLayout()

	ax_week = Axis(grid[1,1])
	ax_weather = Axis(grid[1,2])
	ax_remarks = Axis(grid[2,1])
	ax_event = Axis(grid[2,2],)

	# 曜日 vs. 販売数
	ax_week.xlabel = "曜日"
	ax_week.xticks = (1:5, unique(train.week))
	ax_week.ylabel = "販売数（目的変数）"
	boxplot!(ax_week, train.week_num, train.y, color=(:blue, 0.5), show_notch=true)

	# 天気 vs. 販売数
	ax_weather.xlabel = "天気"
	ax_weather.ylabel = "販売数（目的変数）"
	ax_weather.xticks = (1:7, unique(train.weather))
	boxplot!(ax_weather, train.weather_num, train.y, color=(:red, 0.5), show_notch=true)

	# 特記事項 vs. 販売数
	ax_remarks.xlabel = "特記事項"
	ax_remarks.ylabel = "販売数（目的変数）"
	ax_remarks.xticks = (1:7, unique(train.remarks))
	ax_remarks.xticklabelrotation = π/4
	boxplot!(ax_remarks, train.remarks_num, train.y, color=(:green, 0.5), show_notch=true)
	
	# イベント vs. 販売数
	ax_event.xlabel = "イベント"
	ax_event.ylabel = "販売数（目的変数）"
	ax_event.xticks = (1:3, unique(train.event))
	ax_event.xticklabelrotation = π/4
	boxplot!(ax_event, train.event_num, train.y, color=(:cyan, 0.5), show_notch=true)
...
# ╔═╡ 9b42e377-4e7d-4a13-8361-325dda6373db
let
	train_r = filter(:remarks => !=("お楽しみメニュー"), train)
	dateticks = optimize_ticks(train.datetime[1], train.datetime[end])[1]
	fig = Figure(backgroundcolor=:gray95, resolution=(1200, 500))
	ax1 = Axis(fig[1,1])
	...
	lines!(ax1, datetime2rata.(train.datetime), train.y, color=:blue, label="あり")
	lines!(ax1, datetime2rata.(train_r.datetime), train_r.y, color=:red, label="なし")
	ax1.xticks[] = (datetime2rata.(dateticks) , Dates.format.(dateticks, "yyyy-mm"));
	axislegend("お楽しみメニュー")
...
# ╔═╡ f05fb30e-44ed-48e5-a85f-ea17bf8f60e6
train.fun = ifelse.( train.remarks .== "お楽しみメニュー", 1, 0)

# ╔═╡ 3d89668f-ed63-46d7-a180-8bac0c3b7eb5
let
	fig = Figure(backgroundcolor=:gray95, resolution=(1000, 800))
	grid = fig[1,1] = GridLayout()
	ax = Axis(grid[1,1])
	...
	# お楽しみメニューか vs. 販売数
	ax.xlabel = "お楽しみメニューか？"
	ax.xticks = (0:1, ["No", "Yes"])
	ax.ylabel = "販売数（目的変数）"
	boxplot!(ax, train.fun, train.y, color=(:blue, 0.5), show_notch=true)

	fig
end

# ╔═╡ d73c9957-c67b-4af1-8030-c4732da0e820
md"#### メディアン検定"

# ╔═╡ 16d421d8-049a-4f76-8c20-d8acc8efd377


# ╔═╡ 1a99d831-b1eb-4711-97f1-0a6f8ada4a20


# ╔═╡ 5f49d2d7-a009-4f05-80dd-eb3d7a35c43f
...
# ╔═╡ 2100a0e8-c11f-42a6-97ac-e686916b1abb


# ╔═╡ 40d18558-5f6e-4877-ba72-ace44c95dd90


# ╔═╡ 1b720c81-b56b-4ae2-b9e8-c81ef2a51056
