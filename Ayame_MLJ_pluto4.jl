...
# ╔═╡ 3e9c8e06-cf67-11ec-2075-e70cbff55ad4
begin
	using CSV
	using DataFrames
	using MLJ
...
# ╔═╡ 22287394-61ba-45d3-8e3f-1baebda0a964
using CairoMakie

# ╔═╡ ff2b8efe-9a60-4c26-9b29-6c281ffe4e38
using StatsBase: countmap

# ╔═╡ 24917abc-e825-44b5-b732-523e40383603
using MLJLinearModels

# ╔═╡ 12f8f6ef-dc5f-4ea6-a16a-939fdbae6672
using StableRNGs

# ╔═╡ ff087370-3bb4-4527-a94f-1cc58cfce3c9
md"### アヤメデータの読み込みと確認、カラム名の修正"

# ╔═╡ 39905966-66e9-4d35-a48a-6cc34a7e1a85
train = CSV.read("./train.tsv", DataFrame)

# ╔═╡ f6958d2b-6ee5-4895-aa4b-8a438573f52a
test = CSV.read("./test.tsv", DataFrame)

# ╔═╡ e124a693-bf1c-4923-9bc0-eb7fb0be79ac
sub = CSV.read("./sample_submit.csv", DataFrame, header=false)

# ╔═╡ 23853f0e-2396-481d-8d2d-38b23e94a0eb
rename!(train, [:id, :SepalLength, :SepalWidth, :PetalLength, :PetalWidth, :Class])

# ╔═╡ 874e6f18-2c6f-407f-ab8b-a0794ba0110d
rename!(test, [:id, :SepalLength, :SepalWidth, :PetalLength, :PetalWidth])

# ╔═╡ 85b62b3a-63ac-424d-ab40-856eb90bdd17
LocalResource("./iris.JPG")

# ╔═╡ 4c380408-14c9-4944-84b8-5fcfd4d82182
unique(train.Class)

# ╔═╡ 601886a6-f2e7-4243-8c0f-f0952cab2632
gdf = groupby(train, :Class);

# ╔═╡ bd84bb8b-0d5e-4052-89f5-690ce3f88632
let
	subdf_setosa = gdf[1]
	describe(subdf_setosa, :min, :mean, :max)
end

# ╔═╡ 039edefe-f3f4-4c94-be1d-5a221ec78fef
combine(groupby(train, :Class), nrow)

# ╔═╡ 4d7e82ca-63cb-4a8d-b95a-47646e6b83d6
combine(groupby(train, :Class), names(train, Not([:id, :Class])) .=> mean)

# ╔═╡ 6d54934d-84ed-4aaf-a73f-269a5cae3627
combine(groupby(train, :Class), names(train, Not([:id, :Class])) .=> std)

# ╔═╡ 9ba7c618-20fd-4a82-88fb-6a082566cfc0
y, X_train = unpack(train, ==(:Class))

# ╔═╡ 52fbf557-b1dc-4444-bfc4-9f4286ce5913
X_test = test

# ╔═╡ f51f7678-b0fa-4c24-80f5-57f732bd31e0
md"### ロジスティック回帰"

# ╔═╡ 6126a38c-d168-4893-b098-b5b014be97f5
y_r = coerce(y, OrderedFactor)

# ╔═╡ 029035a5-94b7-4de0-919e-efbd81f30d89
classes(y_r[1])

# ╔═╡ 006db9fe-46f5-4381-81b7-2ccc0f512ac8
cm = countmap(y)

# ╔═╡ 7cf47c36-563e-4b10-8bcc-36cf29845d4c
let
	fig = Figure(resolution=(800,600))

	x = 1:length(cm)
	# class_name = unique(train.Class)
	class_name = combine(groupby(train, :Class), nrow)[:,1]
	class_counts = combine(groupby(train, :Class), nrow)[:,2]
	
	ax = Axis(fig[1,1], xticks=(x, class_name), ylabel="Number [#]", title = "Class")
	
	barplot!(ax, x, class_counts; color=:blue, width=0.8)

	fig
end

# ╔═╡ 100a021e-c554-47c4-9d97-dded366600ab
md"### ロジスティック回帰でモデルを作る"

# ╔═╡ 3b61ba1d-4261-4257-b926-a1dc8af96c47
LogisticClassifier = @load LogisticClassifier pkg=MLJLinearModels

# ╔═╡ 74904338-b603-477f-858d-12a5ceb5dcf1
lcr_model = LogisticClassifier()

# ╔═╡ a041a692-36c4-46dd-91fb-245673c83bba
X = select(X_train, Not(:id))

# ╔═╡ 9868e593-e4fd-4be9-9405-83c6a87e002e
lcr = machine(lcr_model, X, y_r)

# ╔═╡ 41ef2017-795d-47f9-9054-c92e48219426
md"### 訓練データと検証データに分割する"

# ╔═╡ ba3ee66d-4cc7-4e5b-930a-33a75a08e0ec
begin
	rng = StableRNG(566)
	y_train, y_test = partition(eachindex(y_r), 0.7, shuffle=true, rng=rng)
end

# ╔═╡ f67ec5b3-01aa-4dd0-adba-bb40822c0a80
y_test[1:3]

# ╔═╡ b75171a7-90c4-4ae3-8219-3d28748c2fb6
md"### フィットして検証"

# ╔═╡ 4c3d739c-6958-409b-b34f-890e214cf481
fit!(lcr, rows=y_train)

# ╔═╡ a6ae2a24-40e5-4b8a-805a-13a2fd123a00
fitted_params(lcr)

# ╔═╡ 5847473d-0ad5-4346-96fe-65ff5134e79c
ŷ = predict(lcr, rows=y_test)

# ╔═╡ 3656e863-2652-4b2f-aae8-0518235ca03c
cross_entropy(ŷ, y[y_test]) |> mean |> x -> round(x, digits=4)

# ╔═╡ 0ed80c61-9b17-4f0f-b0bc-9656b610cf55


# ╔═╡ 85b9745b-c37d-4b94-b361-cc3a500471ed


# ╔═╡ cb17aa19-0136-4a44-9d86-5362a3fdadd3


# ╔═╡ bdbadcda-1634-42b7-9c7c-fb16a5d80e73


# ╔═╡ 9448287a-692a-49e2-ae5b-3411c838fda5


# ╔═╡ 8b538d9f-1972-42c7-9b16-d90a67b7a825


# ╔═╡ fbbfaf32-1a37-4443-ad48-4b735ea2c169


# ╔═╡ f6a8473e-e183-410f-a500-5275124a56f2
...
git-tree-sha1 = "ebfb85e407e138440b0ee9cb7a5c32e5fa4b887e"
